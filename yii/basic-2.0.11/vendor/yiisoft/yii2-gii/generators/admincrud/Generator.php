...
namespace yii\gii\generators\admincrud;
...
class Generator extends \yii\gii\Generator {
    ...
    public $modelClass = 'app\modules\admin\models\XXX';
    ...
    public $title;
    public $searchModelClass = 'app\modules\admin\models\XXXSearch';
    ...
    public function save($files, $answers, &$results) {
        if (parent::save($files, $answers, $results)) {
            ...
            $db = Yii::$app->db;
            $time = time();
            if (!$db->createCommand('SELECT * FROM auth_item WHERE name=:name')->bindValue(':name', "$ctrlId/index")->queryOne()) {
                $db->createCommand()->batchInsert('auth_item', ['name', 'type', 'description', 'created_at', 'updated_at'], [
                    ["$ctrlId/index", 2, $this->title . '管理', $time, $time],
                    ["$ctrlId/create", 2, $this->title . '添加', $time, $time],
                    ["$ctrlId/update", 2, $this->title . '修改', $time, $time],
                    ["$ctrlId/view", 2, $this->title . '查看', $time, $time],
                    ["$ctrlId/delete", 2, $this->title . '删除', $time, $time],
                ])->execute();
                //添加菜单
                $db->createCommand()->insert('menu', ['fid' => 0, 'title' => $this->title . '管理', 'perm' => "$ctrlId/index"]);
            }
            return true;
        }
        return false;
    ...
    public function rules() {
        return array_merge(parent::rules(), [
            ...
            [['modelClass', 'controllerClass', 'title', 'baseControllerClass', 'indexWidgetType'], 'required'],
